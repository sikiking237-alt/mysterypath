...
import {
  Plus, Search, Edit2, Trash2, Eye, Copy, Archive,
  BookOpen, Clock, Users, Star, ChevronDown,
  Grid3x3, List, Filter, Loader2, CheckCircle,
  FileText, AlertCircle, MoreVertical, TrendingUp,
  Calendar, Award, XCircle
} from 'lucide-react';
import { useGetInstructorCoursesQuery } from '../../features/courses/coursesApi';
...
const MyCourses = ({ darkMode }) => {
  ...
  const stats = {
    ...
    drafts: courses?.filter(c => !c.isPublished).length || 0,
    totalStudents: courses?.reduce((acc, c) => acc + (c.students || 0), 0) || 0
  };
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Link to={`/instructor/edit-course/${course.id}`} className={`p-1.5 rounded-lg ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}>
...
const CourseListItem = ({ course, darkMode }) => {
  return (
    <div className={`flex items-center gap-4 p-4 rounded-xl border-2 transition-all hover:shadow-lg ${
      ...
      <div className="flex gap-2">
        <Link to={`/instructor/edit-course/${course.id}`} className={`p-2 rounded-lg ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}>
          <Edit2 className="w-4 h-4 text-blue-500" />
        </Link>
        <Link to={`/course/${course.id}`} className={`p-2 rounded-lg ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}>
